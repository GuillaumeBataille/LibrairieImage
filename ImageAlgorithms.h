...
void print_RAG(std::vector<Node*>& leaves, std::vector<std::pair<Node*, Node*>>& edges) {
    ...
    for (auto edge : edges) {
        std::cout << "(" << edge.first->coords.first << "," << edge.first->coords.second << ") -> (" <<
            edge.second->coords.first << "," << edge.second->coords.second << ")" << std::endl;
    }
    ...
    for (Node* leaf : leaves) {
        for (auto edge : edges) {
            if (edge.first == leaf) {
                ...
                if (block_distance(leaf, nb) < seuil) { // Si les deux blocs sont assez similaires, on les mélange
